...
use safeshell::{dialog, ShellContext};
...
fn run_stash(shell_context: &ShellContext) -> Result<safeshell::data::CmdExit> {
...
fn run_pop(shell_context: &ShellContext) -> Result<safeshell::data::CmdExit> {
    if !shell_context.is_stash_file_exists()? {
        return Ok(safeshell::data::CmdExit {
            code: 1,
...
pub fn run_restore(shell_context: &ShellContext) -> Result<safeshell::data::CmdExit> {
