...
use safeshell::{config::Config, data::Detection, dialog};
...
pub fn run(subcommand_matches: &ArgMatches, config: &Config) -> Result<safeshell::data::CmdExit> {
...
fn run_create_config(config: &Config) -> Result<safeshell::data::CmdExit> {
...
fn run_validate(config: &Config) -> safeshell::data::CmdExit {
    ...
    safeshell::data::CmdExit {
        code: error_found,
        message: Some(result.join("\n\r")),
    }
}

fn run_delete(config: &Config, force: bool) -> Result<safeshell::data::CmdExit> {
    if !force
...
fn run_ignore(config: &Config) -> Result<safeshell::data::CmdExit> {
    if !config.is_app_path_exists() {
